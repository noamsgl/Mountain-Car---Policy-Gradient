...
def calc_feature(x, cov):
    """ feature function: Gaussian
        x is the distance of the state from some center.
        cov is the co-variance matrix
    """
    return np.exp(-(x.T @ np.linalg.inv(cov) @ x) / 2)


def get_feature_vec(s, centers, cov):
    """ Calculates the featurs vector.
        Given the state , centers - matrix (#features_number X #feature_dimensions) & co-variance matrix (same to every feature).
        For each feature - uses calc_feature(*) for calculation.
        returns the feature vector for the current satate (position, velocity)
    """
    x = s - centers
    phi = np.zeros((len(centers), 1))  # feature vector
    # calc features phi
    for i in range(len(phi)):
        phi[i] = calc_feature(x[i].T, cov)
    return phi
...
def features_vector(S):
    return get_feature_vec(S, C_arr, covariance_matrix)
...
def actor_critic(alpha_theta, alpha_w):
    ...
    for t in tqdm(range(int(20000)), desc="Actor-Critic Steps"):
        ...
        temp = get_expected_feature(S, policy)
